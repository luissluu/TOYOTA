<template>
    <div class="bg-gray-700 rounded-lg p-6">
      <h4 class="text-lg font-medium text-white mb-4">Editar Perfil</h4>
      
      <form @submit.prevent="enviarFormulario(guardarPerfil)" class="space-y-4">
        <!-- Nombre -->
        <CampoValidado
          id="nombre"
  ...
  // Validación adicional para la fecha de nacimiento
  ...
  const guardarPerfil = async () => {
    try {
      ...
      emit('guardar', {
        ...formulario,
        id: props.usuario.id || Date.now().toString(), // Preservar ID existente o crear uno nuevo
      });
      
      return true;
    } catch (error) {
      throw new Error('Error al guardar el perfil. Por favor, intenta nuevamente.');
    }
  };
  ...
  // Al montar el componente, si hay datos de usuario, los cargamos en el formulario
  onMounted(() => {
    if (props.usuario) {

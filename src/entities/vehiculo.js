...
class Vehiculo {
    ...
    static async findByVin(vin) {
        try {
            const pool = await getConnection();
            const result = await pool.request()
                .input('vin', mssql.VarChar(50), vin)
    ...
    static async update(id, vehiculo) {
        try {
            ...
            console.error('Error en update de vehiculo:', error); // Para depuración
            throw error;
        }
    }
    ...
    static async delete(id) {
        try {
            ...
            console.error('Error en delete de vehiculo:', error); // Para depuración
            throw error;
        }
    }
}
